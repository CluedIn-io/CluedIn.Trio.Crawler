...
{
    public class GetCrawlJobDataBehaviour : TrioProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(TrioConstants.KeyName.ApiKey, nameof(TrioCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task TrioCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Trio.Core.TrioCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
