...
namespace CluedIn.Provider.Trio
{
    public class TrioProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly ITrioClientFactory _trioClientFactory;

        public TrioProvider([NotNull] ApplicationContext appContext, ITrioClientFactory trioClientFactory)
            : base(appContext, TrioConstants.CreateProviderMetadata())
        {
            _trioClientFactory = trioClientFactory;
        }
            ...
            Guid providerDefinitionId)
        {
            ...
            if (jobData is TrioCrawlJobData trioCrawlJobData)
            {
                ...
                dictionary.Add(TrioConstants.KeyName.ApiKey, trioCrawlJobData.ApiKey);
            }
        ...
        public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var client = _trioClientFactory.CreateNew(trioCrawlJobData);
        ...
        public string Icon => TrioConstants.IconResourceName;
        public string Domain { get; } = TrioConstants.Uri;
        public string About { get; } = TrioConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = TrioConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(TrioConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(TrioConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = TrioConstants.Instructions,
            Value = new List<string> { TrioConstants.CrawlerDescription },
            Details = TrioConstants.Details

        };

        public string Details { get; set; } = TrioConstants.Details;
        public string Category { get; set; } = TrioConstants.Category;
        public new IntegrationType Type { get; set; } = TrioConstants.Type;
    }
}
